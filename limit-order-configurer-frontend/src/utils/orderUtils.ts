...
function trim0x(bigNumber: string | bigint | number): string {
...
function setn(num: string | bigint | number, bit: string | bigint | number, value: boolean): bigint {
    if (value) {
    ...
    return {
        ...
        postInteraction: ethers.solidityPacked(
            ...
            [feeTaker, makerReceiver ? '0x01' : '0x00', integratorFeeRecipient, protocolFeeRecipient].concat(
                ...
                [integratorFee.toString(), integratorShare.toString(), resolverFee.toString(), whitelistDiscount.toString(), whitelistPostInteraction, customPostInteraction],
            ),
        ),
    };
}
...
interface OrderExtensionsOptions {
...
interface OrderExtensionsFullOptions extends OrderExtensionsOptions {
    customData?: string;
}
...
function buildOrder(
    ...
    if (trim0x(extension).length > 0) {
        salt = BigInt(ethers.keccak256(extension)) & ((1n << 160n) - 1n); // Use 160 bit of extension hash
        makerTraits = (BigInt(makerTraits) | (1n << _HAS_EXTENSION_FLAG)).toString();
    }

    if (trim0x(preInteraction).length > 0) {
        makerTraits = (BigInt(makerTraits) | (1n << _NEED_PREINTERACTION_FLAG)).toString();
    }

    if (trim0x(postInteraction).length > 0) {
        makerTraits = (BigInt(makerTraits) | (1n << _NEED_POSTINTERACTION_FLAG)).toString();
    }
...
function buildOrderData(chainId: number, verifyingContract: string, order: any) {
    return {
...
async function signOrder(order: any, chainId: number, target: string, wallet: ethers.Wallet | ethers.Signer) {
...
function fillWithMakingAmount(amount: string | bigint): bigint {
    return BigInt(amount) | BigInt(buildTakerTraits({ makingAmount: true }).traits);
}

function unwrapWethTaker(amount: string | bigint): bigint {
    return BigInt(amount) | BigInt(buildTakerTraits({ unwrapWeth: true }).traits);
}

function skipMakerPermit(amount: string | bigint): bigint {
